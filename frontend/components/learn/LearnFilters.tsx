...
export function LearnFilters({ filters, onFiltersChange }: LearnFiltersProps) {
  ...
  const hasActiveFilters =
    filters.search || filters.difficulty || filters.sortBy !== 'popular';
          ...
          >
            ...
            {hasActiveFilters && (
              <span className="ml-2 px-2 py-0.5 bg-purple-500 rounded-full text-xs">
                {[filters.difficulty, filters.sortBy !== 'popular' ? 'sort' : '']
          ...
          <div className="flex flex-wrap gap-2">
            {filters.difficulty && (
              <span className="px-3 py-1 glassmorphic rounded-full text-sm text-white/80 flex items-center gap-2">
                Level: {DIFFICULTY_OPTIONS.find((d) => d.value === filters.difficulty)?.label}
                <button
