...
import { HackathonsFilters } from '@/components/hackathons/HackathonsFilters';
...
export default function HackathonsPage() {
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter(
        (hackathon) =>
          ...
          hackathon.tagline?.toLowerCase().includes(searchLower) ||
          hackathon.description.toLowerCase().includes(searchLower) ||
          hackathon.tags.some((tag) => tag.toLowerCase().includes(searchLower))
      );
    }

    // Filtrar por status
    if (filters.status) {
      filtered = filtered.filter((hackathon) => hackathon.status === filters.status);
    }

    // Filtrar por tipo de ubicación
    if (filters.locationType) {
      filtered = filtered.filter((hackathon) => hackathon.locationType === filters.locationType);
    }

    // Filtrar por chain
    if (filters.chain) {
      filtered = filtered.filter((hackathon) => hackathon.chains.includes(filters.chain));
    }
  ...
  return (
    <main className="min-h-screen relative">
      ...
      <Navbar />
      <HackathonsHero />
      <HackathonsFilters filters={filters} onFiltersChange={setFilters} />
      <HackathonsGrid hackathons={filteredHackathons} loading={loading} />
      <Footer />
    </main>
  );
}
